...
class PageClassifier():
	...
	# 	split_text = re.sub("[^\w]", " ",  text).split()
	# 	cleaned_text = [word.lower() for word in split_text if word not in ignored_words]
		...
		for mapping in self.data_structure:
			for key, site in mapping.items():
				for key, page in site.items():
					page = page[0]
					corpus.append("{} {} {}".format(page["short_url"], page["keywords"], page["anchor_tags"], page["h1"]))
	...
	def train_model_about_us_pages(self, corpus, labels):
		cv= CountVectorizer()
		
		mnb = MultinomialNB(alpha=0.1)
		x_train, x_test, y_train, y_test =train_test_split(corpus, labels, test_size=0.3)
		classify= Pipeline([('CountVectorizer', cv), ('naivebayes', mnb)])
		classify.fit(x_train, y_train)
		joblib.dump(classify, 'about_us_model.pkl')
		predict= classify.predict(x_test)
		print("About us page accuracy", accuracy_score(predict, y_test)*100)

	def predict_model_about_us_pages(self, corpus):
		classify= joblib.load('about_us_model.pkl')
		predict= classify.predict(corpus)
		return predict
def main():  #this calls the class and the methods. Coded by @Haks
	...
	parser.add_argument("-t", "--train", help='add a page to train', type=str)
	parser.add_argument("-p", "--predict", help='add a page to train', type=str)
	...
	if args.train:
		...
		corpus, labels, urls = pageclassifier.get_corpus_labels_urls("about")
	
		pageclassifier.train_model_about_us_pages(corpus, labels)
	elif args.predict:
		link = args.predict
		...
		try:
			...
			data_structure = PageAnalyser.data_structure
			pageclassifier = PageClassifier(data_structure)
			corpus, _, urls = pageclassifier.get_corpus_labels_urls("about")

			# predictions = pageclassifier.predict_model_privacy_pages(corpus)
			predictions = pageclassifier.predict_model_about_us_pages(corpus)
